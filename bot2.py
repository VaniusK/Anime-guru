...
async def send_anime(message):
    ...
    if state_user[user_id] == 1:
        if len(user_rate_anime[user_id]) < 1:
            ...
            cur_ls[user_id] = ls
            await bot.send_photo(user_id, photo=df["Image URL"][ls[0]], caption=f'Устраивает ли вас:{name[ls[0]]}?',
    ...
    if state_user[user_id] == 2:
        ...
        if text.isdigit() and int(text) >= 1 and int(text) <= 10:
            ...
            if state_user[user_id] == 5:
                ...
                await bot.send_message(user_id,
                                       f'{name[cur_anime[user_id]]} успешно добавлено в Ваш список с оценкой {text}')
                state_user[user_id] = 1
            cur_anime[user_id] = -1
        else:
